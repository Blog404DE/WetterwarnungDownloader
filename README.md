[![Travis branch](https://img.shields.io/travis/Blog404DE/WetterwarnungDownloader/develop.svg?style=flat-square)](https://travis-ci.org/Blog404DE/WetterwarnungDownloader) [![GitHub release](https://img.shields.io/github/release/Blog404DE/WetterwarnungDownloader.svg?style=flat-square)](https://github.com/Blog404DE/WetterwarnungDownloader) [![GitHub (pre-)release](https://img.shields.io/github/release/Blog404DE/WetterwarnungDownloader/all.svg?style=flat-square)](https://github.com/Blog404DE/WetterwarnungDownloader) [![license](https://img.shields.io/github/license/Blog404DE/WetterwarnungDownloader.svg?style=flat-square)](https://github.com/Blog404DE/WetterwarnungDownloader)
...
1	Falls *composer* noch nicht auf Ihrem System installiert ist, installieren Sie es bitte gemäß der Anleitung unter https://getcomposer.org/download/. Im Anschluss können die Library automatisch geladen werden mittels:
...
3.	Bei Verwendung der Twitter-Action müssen Sie noch das Action entsprechend auskommentieren und konfigurieren. Hierfür sind folgende Konfigurationsparameter zuständig:
    ...
    * Der letzte Parameter *ignoreForceUpdate* ist insbesondere dann wichtig, sofern Sie mehrere "Action"-Klassen aktiviert haben. Normalerweise wird eine Action-Modul erst dann ausgeführt, sobald es zu einer neuen Warnlage kommt. Mit dem Start-Parameter "--force" bzw. "-f" kann man diese Prüfung umgehen und festlegen, dass auf jeden Fall die Ausführung des Action-Moduls stattfinden soll.

        Für einzelne "Action"-Klassen möchte man die erzwungene Ausführung eventuell unterbinden, da man auf keinen Fall mehrfach über eine Warnlage informiert werden möchte. Lässt man sich zum Beispiel über ITFFF per Push-Nachricht oder SMS informieren und möchte gleichzeitig ein Tweet absetzen, so möchte man unter gewissen umständen, dass auf jeden Fall einmal in der Stunde eine Twitter-Nachricht abgesetzt wird, aber auf keinen Fall die Push-Nachricht mehrfach versendet wird.

        Hierfür kann man nun mittels *ignoreForceUpdate* festlegen, dass auf keinen Fall ein weiterer Versand erzwingen werden kann. Stellt man den Parameter von *false* auf *true*, dann wird jegliches "--force"-Kommando ignoriert.
...
2.	Bei Verwendung der IFTTT-Action müssen Sie noch das Action entsprechend auskommentieren und konfigurieren. Hierfür sind folgende Konfigurationsparameter zuständig:
    ...
    * Die Parameter  *MessagePrefix*, *MessagePostfix* und *ignoreForceUpdate* sind identisch zu der Erklärung im vorherigen Abschnitt für das ```SendToTwitter```-Action Modul.
